...
**和 vue 的区别,vue 也是在对比的时候遇到组件，则通过运行 render 函数去得到该组件的虚拟 dom 去依次对比**
**vue 是 tag 相同，key 相同，如果是 input 元素 type 还必须相同，使用头尾指针进行对比**
**若相同,vue 在改变时，直接将属性变化统一**
**react 也是运行类的 render 函数，函数组件是执行该函数，然后得到节点去对比**
**react 中的相同，指类型节点相同，如果是组件 type 必须一致，key 用于同层中的新旧寻找**
**若相同,react 在改变时，记录变化属性，之后统一改变**
